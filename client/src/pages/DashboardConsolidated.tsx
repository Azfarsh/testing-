...
const DashboardConsolidated = () => {
  ...
  const { data: metrics = { totalUsers: 0, activeVendors: 0, totalRevenue: 0, growthRate: 0, userGrowth: 0, vendorGrowth: 0, revenueGrowth: 0, growthRateChange: 0 }, isLoading: isLoadingMetrics, error: metricsError } = useQuery<MetricsData>({
    queryKey: ['/api/metrics'],
  });

  const { data: revenueData = [], isLoading: isLoadingRevenue, error: revenueError } = useQuery<RevenueDataItem[]>({
    queryKey: ['/api/revenue-data'],
  });

  const { data: activities = [], isLoading: isLoadingActivities, error: activitiesError } = useQuery<Activity[]>({
